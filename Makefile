...
UMI_GENERIC_PDF = umi-proquest-form-generic.pdf
...
.PHONY: clean
clean:
	rm -f *.aux *.out *.nav *.toc *.vrb $(PDFS) *.snm *.log *.bbl *.blg *.tex.d *.run.xml *.atfi *.auxlock *.bcf *-blx.bib *.synctex.gz *'.synctex.gz(busy)' *.synctex *'.synctex(busy)' jgross-thesis*figure*.pdf
