...
\todo{Did Automath precede Nqthm?  Curry-Howard}

\todo{This paragraph: The next innovention in automated theorem provers was the ability to write down logical formulae in source code, represented as some sort of datatype.  The automated theorem provers such as EXAMPLES would then apply complete or incomplete decision or search procedures to attempt to find a justification (proof) of the given formulae.}
\todo{Talk about Robinson resolution principle for decision procedure}
\todo{talk about Nqthm and induction}
\todo{Talk about Nqthm used for proving correctness of assembling and executing code, cf \url{https://www.cs.utexas.edu/~moore/best-ideas/piton/index.html}}
\todo{Talk about Computational Logic Inc.~which used Nqthm, \url{http://www.computationallogic.com/}}
Shortly thereafter, automated theorem proving graduated to proving \todo{FIXME talk about logic proving, talk about induction in Nqthm~\cite{Automation2013Moore}}

\todo{Talk about other early theorem provers: Mizar}

\todo{Next paragraph is LCF, the de Bruijn criterion, talk about the LCF approach, and the ability to not store entire proofs.  Maybe talk about Automath before LCF???}

\todo{Paragraph on Nuprl.  Innovation: type theory and proof checking is type checking (but type checking is undecidable).  Poke around Nuprl project website for 10 minutes to see what they highlight.  Talk about Curry-Howard, Check Martin-Lof.  Maybe email Bob Constable for historical context of Nuprl}

\todo{Next paragraph is on Isabelle/HOL, HOL Light, HOL4}

\todo{Next paragraph talks about LEGO (which comes before Coq, makes same foundational decisions, email Randy Pollack(sp?) creator of LEGO, to get historical context), Coq, (+Agda, etc), (innovation: Martin Lof, proof checking is type checking)}

\todo{Talk about remaining misc theorem provers and survey papers}


\todo{Read from page 128 of \textcite{ringer2020qed}}
\todoask{Has anything interesting been done with the Stanford Pascal Verifier~\cite{luckham1979stanford}?  Should I say anything about it?  Adam says maybe not?}
\todo{talk about LCF approach~\cite{harrison2001lcf}}
\todo{wide look at various proof assistants (historical overview of how the field got to where it is today, maybe with weight according to importance) and successes (applications of proof assistants like compcert~\cite{Compcert}, sel4 (kernel)~\cite{seL4SOSP09}, cakeml~\cite{CakeML}, nqthm~\cite{Nqthm,wiki:Nqthm,Automation2013Moore}, ACL2~\cite{ACL2,ACL2Applications}, crypto???, flyspeck~\cite{flyspeck,flyspeck2014Hales}, 4-color-theorem~\cite{gonthier2008formal}, odd-order-theorem~\cite{gonthier2013machine})}
\todo{LCF~\cite{LCF2000Gordon,gordon1979edinburgh,gordon1978metalanguage}, Isabelle/HOL~\cite{LCF2019Paulson,Isabelle/Isar2002Wenzel,Isabelle,paulson1994isabelle}, HOL4~\cite{slind2008brief}, HOL Light~\cite{harrison1996hol}, LF~\cite{pfenning1991logic,harper93}, Twelf~\cite{pfenning1999system}, Matita~\cite{asperti2007user,Matita2011Asperti}, PVS~\cite{owre1996pvs,PVS1992Owre}, Mizar~\cite{harrison-mizar}, Automath~\cite{wiki:AutoMath,mathematical1970Bruijn,Survey1994deBruijn}}
...
\todo{Mention that more details are available in \textcite{Brief2019Darbari,Automation2013Moore}}

\todo{goals: simultaneously let people know that there's been a lot of thought and effort, let people know about success stories (because it possibly hasn't touched them, and hasn't been adopted mainstream), (and also a bit about how the proof assistants might actually check the proofs)}
\todo{The structure of this section:}
\begin{itemize}
\item \todo{how the first proof assistants worked}
\item \todo{subsubsection on LCF approach, and how that lets you prove things}
\item \todo{paragraph on descendents of LCF, dependently typed and not; foreshadow how proof checking as typechecking works with a single sentence or so}
\item \todo{very brief paragraph on other proof assistants, also mention survey papers}
\item \todo{paragraph on successes of proof assistants}
\end{itemize}
...
The first is the use of dependent type theory as a basis for formal proofs, as is done in Coq~\cite{coq}, Agda~\cite{Dependently2009Norell}, Idris~\cite{Idris2013Brady}, Lean~\cite{Lean2015Moura}, Nuprl~\cite{nuprl}, Matita~\cite{Matita2011Asperti}, and others, rather than on some other logic, as is done in
...
\todo{``Another variation of the basic architecture that is part of the Coq proof assistant is the inclusion of aconversion rulein its logical core.  Arguments that are based solely on computation of functions definedin the logic, such as the fact that 1+1=2, are ubiquitous throughout formal proofs.  If we record allthe steps required to show that such arguments are valid as part of the proof objects,  their size soonbecomes prohibitively large.  It has been argued that such arguments should not be recorded in formalproofs but rather should be automatically decided through computation – a principle that has been called the Poincaré principle'' \cite[p.~1167]{barendregt2001proof} \cite{stampoulis2013veriml}}
