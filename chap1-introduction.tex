...
As we come to increasingly rely on large and complex software systems, bugs in these systems have an ever more severe and detrimental impact on our world.
While small bugs can cost hours or days of developer time, other bugs have more severe costs in money or even lives~\cite{zhivich2009real}.
Intel lost about \$475 million when it recalled chips impacted by a minor bug in the lookup table implementing floating-point division~\cite{Infamous2014Gawron,Truth1995Halfhill,Pentium2011Nicely}.
NASA and other space agencies lost hardware costing billions of US dollars due to bugs in rocket software that created unrecoverable errors~\cite{Metric1999Lloyd,Design1997Jazequel,Ariane1997Dowson,Space1996Leech}.
...
%More recently, unforeseen edge-cases in DAO smart-contracts nearly cost investors over 40 million USD~\cite{DAO2018Guecluetuerk}.
Finally, the Therac-25 radiation therapy machine killed three people and injured more by delivering dangerous doses of radiation~\cites[p.~434]{Gift2018Baase}{Investigation1993Leveson}.

While testing of various kinds is the de facto standard for finding bugs, it is both quite costly and does not guarantee a lack of bugs.
\minortodo{email Xavier Leroy and ask if he has references on how much the airline industry spends on testing}
In adversarial contexts, fixing even 99\% of bugs is not sufficient; there is a strong incentive for the remaining 1\% of bugs to be found and exploited.
In some such contexts, random testing cannot hope to eliminate all bugs.
...
Let us envision what numbers this big actually mean.
...
While making our computers sufficiently faster might seem to solve this problem, in fact it gets us nowhere!
Ensuring security would require scaling up the size of the cryptographic problem by a similar amount---otherwise attackers could use these faster computers to brute force the secrets---and we would be no closer to being able to exhaustively test for bugs.
...
We do this by specifying in formal mathematics the intended behavior of our software.
We then show a correspondence between our math and our code.
...
Tools for machine-checked proofs have already had many successes, in both software verification and traditional mathematics proof formalization.
...
%while there is a great deal of research on the topic of efficiently producing proofs in a fully automated way~ these tools, at least at present, cannot hope to solve the challenges
...
\minortodo{work on this paragraph}
...
\todonz{Seems fine ... but is it really unique to proofs?  C++ compilers in the 90s had the same problems}
...
\todoaskreaders{Is this paragraph in particular readable?  Any comments on design choices here?}
...
Nqthm later evolved into ACL2~\cite{Milestones2019Moore,ACL2Applications,ACL2}\todonz{? blank id?}, which has been used, among other things, to verify a Motorola digital signal processor, the floating-point arithmetic unit in AMD chips, and some x86 machine code programs~\cite[p.~2]{Milestones2019Moore}.
...
\minortodo{Should I say this? We refer readers to \textcite{CoqArtForward2013Huet} for a comprehensive history of the development of Coq.}
...
\minortodo{should I mention formal verification of Nuprl in Coq?}
...
In the remainder of \autoref{part:introduction}, we will finish laying out the landscape of performance bottlenecks in dependently-typed proof assistants;
...
The category theory library formalized as part of this doctoral work, available at \textcite{HoTT/HoTT-categories}\todonz{blank citation}, is described briefly in this chapter; a more thorough description can be found in the paper we published on our experience formalizing this library~\cite{category-coq-experience}.
...
\fullref{ch:rewriting} describes our original contribution of a framework for leveraging reflection to perform rewriting and program transformation at scale, driven by our need to synthesize efficient, proven-correct, low-level cryptographic primitives~\cite{FiatCryptoSP19}.
