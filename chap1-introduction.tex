...
However, in all of these tools, there is enormous proof overhead over the lines of code being verified. (Insert graph) 

To decrease the marginal overhead of proof generation, we may automate the proof generation itself. 
Drawing experience from case studies in category theory~\cite{category-coq-experience}, parsers~\cite{jgross-masters-thesis}, and generation of low-level cryptographic code~\cite{FiatCryptoSP19}, we present the difficulties in automation of proof generation. 
In the quest to build automated proof generation tools, we encountered several performance bottlenecks in proof-checking. In this thesis, we present design principles in \autoref{part:design} to avoid these performance bottlenecks, and insights about the proof-checking system that the performance bottlenecks reveal. We present a research prototype of a tool and methodology for achieving acceptable performance at scale in \autoref{part:rewriting}. 
