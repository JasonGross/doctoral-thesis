...
Consider the problem of typechecking \mintinline{coq}{@eq\_refl nat (fact 100) : @id nat (fact 100) = fact 100}, where \mintinline{coq}{fact} is the factorial function on natural numbers and \mintinline{coq}{id} is the polymorphic identity function.
...
\subsubsection{Quadratic Creation of Substitutions for Existential Variables}
...
\subsubsection{Quadratic Substitution in Function Application}
...
\subsubsection{Quadratic Normalization by Evaluation}
...
\subsubsection{Quadratic Closure Compilation}
...
When doing optimized reduction, one operation that might need to happen is the computation of free variables of abstraction nodes.
\todo{figure out why, and explain it}
If this computation of free variables is done in the na\"ive way, it'll be linear in the size of the term.
If this is done recursively for every abstraction node, then we get a quadratic blowup in the number of nested abstraction nodes.
...
\todo{Add new explanation of closure compilation; talk about chaining reflective passes}

\todo{maybe more examples here?}

\subsection{The Number of Abstraction Barriers}
\begin{itemize}
\item \todo{introduce abstraction barrier section}
\item \todo{talk about packed vs unpacked records}
\item \todo{talk about opportunity for conversion to make the wrong choice increasing with number of abstraction barriers}
\end{itemize}
