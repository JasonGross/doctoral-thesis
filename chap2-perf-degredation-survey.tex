...
Let us now consider what it takes to typecheck a slightly smaller version of the example from above.
Let us consider a s
\begin{minted}{coq}
(λ (T0 : Prop) (v0 : T0),
  (λ (T1 : Prop) (v1 : T1), (and True T1) (@conj True T1 I v1))
    (and True T0) (@conj True T0 I v0))
  True I
\end{minted}
We need to typecheck the following expressions:
\begin{enumerate}
    \item
    Check that \mintinline{coq}{True : Prop} in the current context
    \item
    Add \mintinline{coq}{T0 : Prop := True} to the current context and check that
\end{enumerate}
we must first check that \mintinline{coq}{True : Prop} and that in the context where \mintinline{coq}{T0 : Prop := True}


\todo{this subsection}


However, most na\"ive implementations of type theory do not 
