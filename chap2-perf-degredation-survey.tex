...
As alluded to in \autoref{sec:manual-proof-long}, when writing non-automated\todoac{crossed out - here [Jason asks: what should it be instead?]} proofs to verify code in a proof assistant, the number of lines of proof we need to write scales with the number of lines of code being verified, typically resulting to a $10\times$ to $100\times$ overhead.
...
Automating the generation of proofs solves the overhead\todoac{Can one ``solve'' an ``overhead''?} of proof-writing time; however, it introduces massive \emph{nonlinear} overhead in the time it takes for the computer to generate and check the proof.
...
(Our primary non-toy\todoac{the - is crossed out [Jason responds: what should be here?]} test example used four machine words and took just under a minute; the biggest realistic example we were targeting was twice that size, at eight machine words, and took about 20 hours.)
...
\todoac{[About this entire paragraph] Subtle point here that might benefit from more explanation/example! [Jason: How much more? / What should I say?]}
...
\todoac{Another bit of evidence that it isn't worth using wrapfig vs. normal figures in a dissertation! [Response from Jason: But I don't know how to get figures that aren't way too tall/big without using wrapfig]}
...
Note that fully nameless, i.e., de Bruijn term representations,\todoac{remove comma [Jason: I think that's wrong]} do not suffer from this issue.
...
\todoac{to (but that still doesn't seem enough for the sentence to make sense) [Jason: I chose ``for'' instead, it makes sense to me?]}
...
\item
  ...
  The class of objects is generally denoted $\Ob[\mathcal C]$\todoac{insert ``,'' here [Jason: I think you've told me to remove commas in places like this?]} and the class of morphisms between $x, y\in \Ob[\mathcal C]$ is generally denoted $\Hom[\mathcal C](x, y)$.
...
\end{itemize}
Although we eventually go into a bit more of the detail of these definitions throughout \autoref{sec:category-def}, we advise the interested reader to consult the rich existing literature on category theory,\todoac{It could be a bit dangerous to suggest that arbitrary readers pause here to go learn category theory! [Jason says: not sure how to rephrase]} including for example \textcite{awodey2010category} and \textcite{mac1998categories}.
...
This is more than 200$\times$ slower than defining this data\todoac{This wording throws me for a loop. The fact that ``data'' is plural is probably only part of the reason.} in the particular case of the category of sets.%
\footnote{%
