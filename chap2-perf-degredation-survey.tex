...
\begin{figure*}
    ...
    \begin{axis}[xlabel=\# limbs,
        ...
        \addplot [no markers, black] gnuplot [raw gnuplot] { % allows arbitrary gnuplot commands
            f(x) = a*exp(b*x);     % Define the function to fit
            a=0.0001;b=2.5;        % Set reasonable starting values here
            fit f(x) 'fiat-crypto-perf-logs/montgomery-fesub-before.txt' u 1:3 via a,b; % Select the file, starts at col 1 and two variables
            plot [x=1:9] f(x);    % Specify the range to plot
            set print "montgomery-fesub-before-user-parameters.dat";  % Open a file to save the parameters
            print a, b;                  % Write the parameters to file
        };
        \addlegendentry{synthesis}
        \addlegendentry{\pgfplotstableread{montgomery-fesub-before-user-parameters.dat}\parameters % Open the file Gnuplot wrote
            ...
            $\pgfmathprintnumber{\paramA} e^{\pgfmathprintnumber{\paramB} (\text{\# limbs})} $
        }
    \end{axis}
\end{tikzpicture}
...
\begin{figure*}
    ...
    \begin{axis}[xlabel=\# limbs,
        ...
        \addlegendentry{\pgfplotstableread{montgomery-fesub-before-user-parameters.dat}\parameters % Open the file Gnuplot wrote
            \pgfplotstablegetelem{0}{0}\of\parameters \pgfmathsetmacro\paramA{\pgfplotsretval} % Get first element, save into \paramA
            \pgfplotstablegetelem{0}{1}\of\parameters \pgfmathsetmacro\paramB{\pgfplotsretval}
            $\pgfmathprintnumber{\paramA} e^{\pgfmathprintnumber{\paramB} x} $
        }
    \end{axis}
\end{tikzpicture}
...
\begin{figure*}
    ...
    \begin{axis}[xlabel=\# limbs,
        ...
        \addplot [no markers, black] gnuplot [raw gnuplot] { % allows arbitrary gnuplot commands
            f(x) = a*exp(b*x);     % Define the function to fit
            a=10;b=0.2;            % Set reasonable starting values here
            fit f(x) 'fiat-crypto-perf-logs/montgomery-fesub-after.txt' u 1:3 via a,b; % Select the file, starts at col 1 and two variables
            plot [x=1:17] f(x);    % Specify the range to plot
            set print "montgomery-fesub-after-user-parameters.dat";  % Open a file to save the parameters
            print a, b;                  % Write the parameters to file
        };
        \addlegendentry{synthesis}
        \addlegendentry{\pgfplotstableread{montgomery-fesub-after-user-parameters.dat}\parameters % Open the file Gnuplot wrote
            \pgfplotstablegetelem{0}{0}\of\parameters \pgfmathsetmacro\paramA{\pgfplotsretval} % Get first element, save into \paramA
            \pgfplotstablegetelem{0}{1}\of\parameters \pgfmathsetmacro\paramB{\pgfplotsretval}
            $\pgfmathprintnumber{\paramA} e^{\pgfmathprintnumber{\paramB} (\text{\# limbs})} $
        }
