\chapter{\readyforreadingmod{The Performance Landscape in Type-Theoretic Proof Assistants}{minor word edits}} \label{ch:perf-failures}
...
Automating the generation of proofs resolves the issue of overhead of proof-writing time.
Automation significantly decreases the marginal cost of proving theorems about new code that is similar enough to code already verified.
However, it introduces massive \emph{nonlinear} overhead in the time it takes for the computer to generate and check the proof.
...
\begin{wrapfigure}[24]{r}{.5\textwidth}
  \centering
    ...
    \begin{axis}[title={synthesis time (s) vs.\ \# limbs},
...
\end{tikzpicture}
    ...
    \begin{axis}[title={synthesis time (s) ($\log$-scale) vs.\ \# limbs},
%        legend pos=north west,
        width=0.4\textwidth,
        axis lines=left,
        xmin=0,
...
\end{tikzpicture}
    \caption{%Timing of
Synthesizing Subtraction\protect\minortodonz{somewhat hard to make sense of these graphs without more explanation...}} \label{fig:timing-montgomery-fesub-before-user}
\end{wrapfigure}
...
(Our primary nontoy test example used four machine words and took just under a minute; the biggest realistic example we were targeting was twice that size, at eight machine words, and took about 20 hours.)
...
\minortodo{adjust spacing of figure}%
...
\begin{figure}
  \centering
  \newsavebox{\RepeatConstructorSubfloat}%
  \sbox{\RepeatConstructorSubfloat}{%
    \adjustbox{valign=t}{%
    \beginTikzpictureStamped{
...
%        legend pos=north west,
        ...
        \addlegendentry{
            \tiny$\pgfmathprintnumber{\pgfplotstableregressiona}n^2
...
\end{tikzpicture}
  ...
  \newsavebox{\ConjTrueLtacTwoSubfloat}%
  \sbox{\ConjTrueLtacTwoSubfloat}{%
    \adjustbox{valign=t}{%
  \beginTikzpictureStamped{
...
%        legend pos=north west,
        ...
        \addlegendentry{
            \tiny$\pgfmathprintnumber{\builda}n
  ...
  \newcommand{\vphantomSubfloat}{\vphantom{\usebox{\RepeatConstructorSubfloat}}\vphantom{\usebox{\ConjTrueLtacTwoSubfloat}}}%
  \subfloat[Timing of \mintinline{coq}{repeat constructor}]{\usebox{\RepeatConstructorSubfloat}\vphantomSubfloat\label{fig:timing-conj-True-repeat-constructor}}%
  \quad
  \subfloat[Timing of manually building and typechecking a certificate using \LtacTwo]{\usebox{\ConjTrueLtacTwoSubfloat}\vphantomSubfloat\label{fig:timing-conj-True-ltac2}}
\caption{Proving a conjunction of $n$ \mintinline{coq}{True}s}\label{fig:timing-conj-True}
\end{figure}

Note that for small and even medium-sized examples, it's pretty reasonable to do duplicative work.
It's only when we reach very large examples that we start hitting nonlinear behavior.
...
There are two ways for the user to share subterms: using \mintinline{coq}{let} binders and using function abstraction.
...
\begin{figure}
  \subfloat[using \mintinline{coq}{let} binders]{%
    \beginTikzpictureStamped{
...
%        legend pos=north west,
    ...
    \end{axis}
\end{tikzpicture}
\label{fig:timing-conj-True-let-ltac2}}
  \quad
  \subfloat[using abstraction and application]{%
    \beginTikzpictureStamped{
...
%        legend pos=north west,
    ...
    \end{axis}
    \end{tikzpicture}
\label{fig:timing-conj-True-app-ltac2}}
  \caption{Timing of manually building and typechecking a certificate to prove a conjunction of $n$ \mintinline{coq}{True}s using \LtacTwo}\label{fig:timing-conj-True-let-app-ltac2}
\end{figure}
...
For example, suppose the function \mintinline{coq}{f TM n} takes as arguments a description of a Turing machine \mintinline{coq}{TM} and a number of steps \mintinline{coq}{n} and outputs \mintinline{coq}{false} unless \mintinline{coq}{TM} halts within \mintinline{coq}{n}, in which case it instead outputs \mintinline{coq}{true}.
Then for any concrete number \mintinline{coq}{n} and any concrete description of a Turing machine \mintinline{coq}{TM} which does in fact halt within \mintinline{coq}{n} steps, the term \mintinline{coq}{@eq_refl bool true} can be typechecked as a proof of \mintinline{coq}{f TM n = true} because \mintinline{coq}{f TM n} computes to \mintinline{coq}{true}.
...
If the typechecker either decides to unfold \mintinline{coq}{id} before unfolding \mintinline{coq}{fact}, or if it performs a breadth-first search, then we get speedy performance.
...
As described in \autoref{sec:fiat-crypto-codegen-numbers}, in Fiat Cryptography, we got exponential slowdown due to this issue, with an estimated overhead of over four thousand millennia of extra typechecking time in the worst examples we were trying to handle.
...
\begin{wrapfigure}{r}{0.5\textwidth}
  \noindent
    ...
    \pgfplotsset{every axis legend/.append style={
        ...
        anchor=north}}
    \small
    \begin{axis}[title={time (s) vs.\ \# binders},
        xlabel=$n$,
%        legend pos=north west,
        width=0.4\textwidth,
        axis lines=left,
...
See \coqbug{9582}; note that Coq 8.10 and later do not show this superlinear behavior due to \coqpr{9586}, and hence our plots in this section use Coq 8.9.1.
...
%        legend pos=north west,
        ...
        \addlegendentry{\tiny parsing and internalization}
    ...
    \end{axis}
\end{tikzpicture}
\caption{Timing of internalizing a function with $n$ differently named arguments of type \mintinline{coq}{True}%
\todo{check on figure spacing}%
} \label{fig:timing-nested-lambda-different-name}
\end{wrapfigure}
...
\footnote{%
  This is done by measuring the time it takes to execute \mintinline{coq}{do 1000 (let v := uconstr:(I) in idtac)}.
}
...
%\minortodoac{Another bit of evidence that it isn't worth using wrapfig vs. normal figures in a dissertation! [Response from Jason: But I don't know how to get figures that aren't way too tall/big without using wrapfig]}
...
\end{tikzpicture}%
  ...
  \newsavebox{\LambdaBindersSubfloat}%
  \sbox{\LambdaBindersSubfloat}{%
    \adjustbox{valign=t}{%
    \beginTikzpictureStamped{
...
%        legend pos=north west,
  ...
  \newcommand{\vphantomSubfloat}{\vphantom{\usebox{\EvarsSubfloat}}\vphantom{\usebox{\LambdaBindersSubfloat}}}%
  \subfloat[Timing of generating 1000 evars in a context of size $n$]{\usebox{\EvarsSubfloat}\vphantomSubfloat\label{fig:timing-do-n-open-constr-True}}%
  \quad
  \subfloat[Timing of generating a $\lambda$ with $n$ binders of unknown/evar type, all of which have either no name, the same name, or different names]{\usebox{\LambdaBindersSubfloat}\vphantomSubfloat\label{fig:timing-open-constr-n-lambda-no-types}}%
\caption{Performance Benchmarks for Substitution}\label{fig:perf:capture-avoiding-subst}
\end{figure}
...
%        legend pos=north west,
        ...
        \addlegendentry{\tiny typechecking}
...
Consider the case of typechecking a nondependent function applied to $n$ arguments.
...
Note that this is important, for example, in attempts to avoid the problem of quadratically sized certificates by making a $n$-ary conjunction constructor which is parameterized on a list of the conjuncts.
Such a function could then be applied to the $n$ proofs of the conjuncts.

We've reported these issues in Coq in \coqbug{8232} and \coqbug{12118}, and a partial solution has been merged in \coqpr{8255}.

See \autoref{fig:timing-app-n} for timing details on a microbenchmark of this bottleneck, where we use \LtacTwo\space to build an application of a function to $n$ arguments of type \mintinline{coq}{unit}.
\LtacTwo\space is a relatively recent successor to the \Ltac\space tactic language, which allows more low-level operations that provide more fine-grained control over what the proof assistant is actually doing.

\minortodonz{[previous two paragraphs] say more}%
...
%        legend pos=north west,
        ...
        \addlegendentry{\tiny \mintinline{coq}{cbv}}
        ...
        \addlegendentry{\tiny\mintinline{coq}{lazy}}
...
\minortodo{cite something for flat closure compilation}%
...
\begin{figure}[bth]
  \centering
  \subfloat[\mintinline{coq}{vm_compute}]{%
    \beginTikzpictureStamped{
...
%        legend pos=north west,
        ...
        \addlegendentry{\tiny \mintinline{coq}{vm_compute}}
    ...
    \end{axis}
\end{tikzpicture}
\label{fig:timing-quadratic-vm-PHOAS}}
  \quad
  \subfloat[\mintinline{coq}{native_compute}]{%
    \beginTikzpictureStamped{
...
%        legend pos=north west,
        ...
        \addlegendentry{\tiny \mintinline{coq}{native_compute}}
    ...
    \end{axis}
\end{tikzpicture}
\label{fig:timing-quadratic-native-PHOAS}}
\caption{Timing of running reduction on interpreting a PHOAS expression as a function of the number of binders}\label{fig:timing-quadratic-vm-native-PHOAS}
\end{figure}
...
\end{itemize}
Although we eventually go into a bit more of the detail of these definitions throughout \autoref{sec:category-def}, we advise the interested reader to consult the rich existing literature on category theory, including for example \textcite{awodey2010category} and \textcite{mac1998categories}.
...
This is more than 200$\times$ slower than it needs to be.
The essential structure needed to define these components can be defined without any of the categorical indirection, taking $\mathcal C₁$, $\mathcal C₂$, and $\mathcal D$ to be types and taking morphisms in these ``categories'' to be proofs of equality between morphism sources and targets.
Defining the structure of the seven components in this way nets us a 200$\times$ speedup!%
\footnote{%
...
We attribute the overhead of the categorical definition to the large types generated and the nontrivial conversion problems which require unfolding various definitions, i.e., piercing various abstraction barriers.
...
\minortodo{How should this chapter be concluded?}%
\minortodo{Maybe another look-forward at what comes next?}%

\begin{subappendices}
\begin{comment}
\section{Comments from Andres}
in chapter 2 you start by comparing coq by nondependently typed languages. perhaps the more appropriate comparison would be between coq and *compilers* of other languages? Ah, you do this later.
...
Thu 7:22pm
and the nonlocality of design choices is even across libraries and proof assistant implementations
Thu 7:22pm
...
\end{comment}
  %If the reader walks away from this thesis with this level of understanding---that there was a superlinear performance bottleneck in the automation of generating verified low-level cryptographic code, and the project presented in \autoref{ch:rewriting} fixed this problem---that is accurate.
...
%I'll have told them that the problem with automation, in my experience, is that it scales superlinearly.

%This chapter aims first to convince the reader that solving these superlinear scaling factors involves more than just ``find the slowness and do the obvious thing to fix it'', and second to demystify the additional structure that's present.
