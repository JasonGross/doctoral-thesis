...
\todoac{Regarding the entire following paragraph: It's strange to put this discussion of performance in *this* chapter, not the one where you show how to reify well. [Note: High Priority]}
...
\begin{wrapfigure}[7]{r}{\widthof{\texttt{| NatMul (x y : expr) : expr.}}}
%\vspace{-45pt}
\begin{minted}{coq}
Inductive expr :=
| NatO : expr
...
\todoac{Weird to get to this point without introducing polymorphically abstracted expression types. The reader may be wondering where the two instantiated ASTs come from, such that we know they parallel each other. [Note: High Priority]}
...
\todoac{Here's the part I expected to see earlier [Note: High Priority]}% N.B. no blank line here when this todo is removed
\begin{minted}{coq}
...
\todoac{This part is interesting for the implicit transition away from ``here are some new tricks that correct deficiencies in prior PHOAS papers.'' At least, my POPL'10 paper *does* produce initial PHOAS well-formedness proofs using a verified-as-well-formed translator from de Bruijn. [Jason's Response: I think the trick here is a bit different (in particular it involves round-tripping rather than just coming from de Bruijn)?  Note also that having a compact representation for this proof term requires that the de Bruijn well-formed predication is a computational function that outputs a Boolean.  Furthermore, indexing the de Bruijn representation over the list of context types (or over the length, as is done in the POPL'10 paper) incurs quadratic overhead in the size of the represented term, so you can't just have this property be true by construction.]}
...
\begin{wrapfigure}[14]{r}{\widthof{\texttt{~~~~~~~~~~~~~constr:(@NatMul v X Y)}}}
\begin{minted}{coq}
...
\todoac{The ``)'' at the end of the previous sentence is crossed off. [Jason: Why?]}%
