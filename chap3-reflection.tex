...
\todoac{Regarding the entire following paragraph: It's strange to put this discussion of performance in *this* chapter, not the one where you show how to reify well.}
...
\todoac{Weird to get to this point without introducing polymorphically abstracted expression types. The reader may be wondering where the two instantiated ASTs come from, such that we know they parallel each other.}
...
\todoac{Here's the part I expected to see earlier}% N.B. no blank line here when this todo is removed
\begin{minted}{coq}
