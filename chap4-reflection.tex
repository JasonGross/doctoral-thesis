...
| RelatedLetIn {Γ e1 e2 f1 f2}
 : related Γ e1 e2 -> (∀ v1 v2, related ((v1, v2) :: Γ) (f1 v1) (f2 v2)) -> related Γ (LetIn e1 f1) (LetIn e2 f2).
\end{minted}
...
\begin{minted}{coq}
Fixpoint is_related_at (P : Prop) {var1 var2 : Type} (Γ : list (var1 * var2)) (e1 : @expr var1) (e2 : @expr var2) : Prop :=
  match e1, e2 with
  | NatO, NatO => P
  | NatS e1, NatS e2 => is_related_at P Γ e1 e2
  | NatMul x1 y1, NatMul x2 y2
    => let F1 := fun P => is_related_at P Γ x1 x2 in
       let F2 := fun P => is_related_at P Γ y1 y2 in
       F1 (F2 P)
  | Var v1, Var v2 => forall n : N, List.nth_error Γ (N.to_nat n) = Some (v1, v2) -> P
  | LetIn e1 f1, LetIn e2 f2
    => let F1 := fun P => is_related_at P Γ e1 e2 in
       let F2 := fun P => forall v1 v2, is_related_at P ((v1, v2) :: Γ) (f1 v1) (f2 v2) in
       F1 (F2 P)
  | _, _ => False
  end.
\end{minted}
We can now prove that \mintinline{coq}{(∀ P, P → is_related_at P Γ e1 e2) → is_related Γ e1 e2}.
By instantiating \mintinline{coq}{P} with \mintinline{coq}{is_related Γ e1 e2} and the first argument with the identity function, we can prove that \mintinline{coq}{
