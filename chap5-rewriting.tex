...
\todoac{More transition language saying ``and now we'll discuss the second one.'' [Note: High Priority]}
...
Note that the type of \mintinline{coq}{eval_decision_tree} would be something like \mintinline{coq}{∀ {T : Type} (d : decision_tree) (ts : list type) (es : exprlist ts) (K : exprlist ts → option T), option T}.\todoac{Mention here that you use CPS for the same reason introduced earlier, though now it is at a more ``meta'' level? [Note: High Priority; remind the reader that CPS is important here to get the right reduction behavior (Also, should there be a nat index for rewrite rules?)]}
