...
\todo{do these before Monday and remove this comment}
\begin{comment}
TODO before Monday:
\todo{Add code example of how CPS helps with reduction of blocked matches}
TODO Yes, cite CPDT about convoy
\todo{include example about recursing into something like $x + (y + 1)$ and $(a + b) + (c * 2)$}
\todo{add code example about +}
\todo{should I discuss why we can't just fuse them?  or how we decide when to put NbE values into rExpr (when they have base types rather than arrow types)?  Adam says yes}
\todo{Add a paragraph about correctness conditions, and how the two sorts of correctness conditions are about denotational semantics, and about syntactic well-formedness, which for PHOAS relates two ASTs with different \mintinline{coq}{var} types}
\end{comment}
...
\todo{Add code example of how CPS helps with reduction of blocked matches (maybe example: mapping 2* over a list choice based on an option)}
...
After breaking apart the first element, we must convoy \todo{cite CPDT or somewhere in this thesis for the convoy pattern?} the continuation across the \mintinline{coq}{match} statement so that we can pass an expression vector of the correct type to the continuation \mintinline{coq}{K}.
...
\todo{include example about recursing into something like $x + (y + 1)$ and $(a + b) + (c * 2)$}
\todo{swap second and thir answers for flow reasons}
...
\todo{should Ie laborate more on the pain?}
...
\todo{does this need to be explained more? How?  Adam says yes}
\todo{add code example about +}
...
\begin{minted}{coq}
Lemma wf_eval_decision_tree {T1 T2} G d (P : option T1 → option T2 → Prop)
 : ∀ (HPNone : P None None)
...
Then we can conlude that the calls to \mintinline{coq}{eval_decision_tree} either both fail, or both succeed with related outputs.
...
At the user-level, this is quite obvious: the left-hand side of the rewrite rule (prior to reification\footnote{%
  Note that this reification is a tactic procedure reifying Gallina to PHOAS, \emph{not} the \mintinline{coq}{reify} function of normalization by evaluation dicussed elsewhere in this chapter.%
