...
\todo{insert references to \cref{foot:break-abstraction:example:decision-tree-indexing} / \autoref{foot:break-abstraction:example:decision-tree-indexing} in \fullnameref{sec:break-abstraction:example:decision-tree-indexing} and to \fullnameref{sec:break-abstraction:example:eval-decision-tree}}
...
We saw in \nameref{sec:abstraction-barriers:mismatch} that API mismatch results in type-size blowup; we saw a particularly striking example of this in \autoref{sec:break-abstraction:example:eval-decision-tree} where an API mismatch resulted in vastly more complicated theorem statements.
...
\todonz{[re this whole section] what is this about?  performance or something else?}
...
We saw in \cref{foot:break-abstraction:example:decision-tree-indexing} in \fullnameref{sec:break-abstraction:example:decision-tree-indexing} how dependent types cause coupling between otherwise unrelated design decisions.
...
In the example of \autoref{sec:break-abstraction:example:decision-tree-indexing}, we considered indexing the list of terms over a list of types, and indexing both this list of types and the decision tree over a natrual number length.
...
If we need to insert an unknown number of elements into the middle of a length-indexed list, the length of the resulting list is not judgmentally the sum of the lengths, because addition is not judgmentally comutative.
