\chapter{\readyforreadingmod{Abstraction}{minor wording edits}}\label{ch:design}\label{ch:api-design}\label{ch:abstraction}

\section{Introduction}
In \Autoref{ch:intro,ch:perf-failures}, we talked about\todoac{``we talked about'' Too informal / different from normal conventions for technical writing} two different fundamental sources of performance bottlenecks in proof assistants:
...
In this chapter, we will dive further into the performance issues arising from the first of these design decisions, expanding on \fullnameref{sec:axis-nested-abstraction-barriers} and proposing some general guidelines for handling these performance bottlenecks.
...
We saw in \nameref{sec:abstraction-barriers:mismatch} that API mismatch results in type-size blowup; we saw a particularly striking example of this in \fullnameref{sec:break-abstraction:example:eval-decision-tree} where an API mismatch resulted in vastly more complicated theorem statements.
...
it is painful\todoac{not ``painful''} to recombine proofs and programs after you\todoac{not ``you''} separate them;
if you\todoac{not ``you''} are doing it to define an opaque transformation that acts on proof-carrying code, that is okay, but if you\todoac{not ``you''} cannot make that abstraction barrier, enormous pain results.

For example, if you\todoac{not ``you''} have length-indexed lists and want to index into them with elements of a finite type, things are fine until you\todoac{not ``you''} need to divorce the index from its proof of finiteness.
If you\todoac{not ``you''}, for example, want to index into, say, the concatenation of two lists, with an index into the first of the lists, then you\todoac{not ``you''} will likely run into trouble, because you\todoac{not ``you''} are trying to consider the index separately from its proof of finitude, but you\todoac{not ``you''} have to recombine them to do the indexing.

We saw in \autoref{foot:break-abstraction:example:decision-tree-indexing} in \fullnameref{sec:break-abstraction:example:decision-tree-indexing} how dependent types cause coupling between otherwise-unrelated design decisions.
...
Although preexisting formalizations of category theory in proof assistants abound~\cite{megacz-coq-categories,Ahrens2013,o2004towards,copumpkin/categories,ConCaT%
,CatsInZFC%%fullbib:
,spitters2010developing%
,kozen2006automating%%fullbib:
,math-overflow-formalizations%
,benediktahrens/coinductives,benediktahrens-Foundations-typesystems,Carvalho1998,jmchapman/restriction-categories,konn/category-agda,crypto-agda/crypto-agda,mattam82-cat,Coalgebras,Algebra,ahrens2010categorical,weber02program,pcapriotti/agda-categories,huet2000constructive,altucher1990mechanically,Category2-AFP,MathClasses,benediktahrens/rezk-completion,mohri1995formalization,spiwackverified%%fullbib:
%,rs-/triangles%
,caccamo2001higher,aczel1993galois,wilander2005bicategory,logical2001implicit,dyckhoff1985category,wilander2012constructing,harrison1996formalized,agerholm1995experiments,nuo2013second%%fullbib:
...
Beginning from scratch allowed me to familiarize myself with both category theory and Coq, without simultaneously having to familiarize myself with a large preexisting code base.
...
%\autoref{sec:duality-conversion} presents one of our favorite design patterns for categorical constructions: a way of coaxing Coq's definitional equality into implementing \emph{proof by duality}, one of the most widely known ideas in category theory.
%In \autoref{sec:other}\todonz{?}, we describe a few other design choices that had large impacts on usability and performance, often of a few orders of magnitude.
%\minortodo{this is duplicative with text above}


\section{A Sampling of Abstraction Barriers}\label{sec:abstraction-barriers}
...
%A few other pervasive strategies made nontrivial differences for proof performance or simplicity.
...
\subsection{Nested Σ Types} \label{sec:nested-sigma-types}
  ...
  This is even more of a problem when we need to \mintinline{coq}{destruct x} to prove something about the projections, as we need to \mintinline{coq}{destruct} it as many times as there are fields, which adds another factor of $n$ to the performance cost of building the proof from scratch; in Coq, this cost is either avoided due to sharing or else is hidden by a quadratic factor with a much larger constant coefficient.
...
%          legend pos=north west,
          ...
          \addlegendentry{\tiny \mintinline{coq}{repeat destruct}}
          ...
          \addlegendentry{\tiny $\pgfmathprintnumber{\destructa}n^2\pgfmathprintnumber[print sign]{\destructb}n\pgfmathprintnumber[print sign]{\destructc}$}
          ...
          \addlegendentry{\tiny \mintinline{coq}{Constr.type} on an \LtacTwo-built projection}
          ...
          \addlegendentry{\tiny $\pgfmathprintnumber{\typea}n^3\pgfmathprintnumber[print sign]{\typeb}n^2\pgfmathprintnumber[print sign]{\typec}n\pgfmathprintnumber[print sign]{\typed}$}
          ...
          \addlegendentry{\tiny building the projection via \mintinline{coq}{cbv}}
          ...
          \addlegendentry{\tiny $\pgfmathprintnumber{\cbva}n^3\pgfmathprintnumber[print sign]{\cbvb}n^2\pgfmathprintnumber[print sign]{\cbvc}n\pgfmathprintnumber[print sign]{\cbvd}$}
      \end{axis}
  \end{tikzpicture}
  \label{fig:timing-make-destruct-nested-sig-no-prim}}
    \quad
    \subfloat[With Primitive Projections]{%
      \beginTikzpictureStamped{
...
%        legend pos=north west,
        ...
        \addlegendentry{\tiny $\pgfmathprintnumber{\typea}n^2\pgfmathprintnumber[print sign]{\typeb}n\pgfmathprintnumber[print sign]{\typec}$}
        ...
        \addlegendentry{\tiny $\pgfmathprintnumber{\cbva}n^2\pgfmathprintnumber[print sign]{\cbvb}n\pgfmathprintnumber[print sign]{\cbvc}$}
    \end{axis}
    \end{tikzpicture}
\label{fig:timing-make-destruct-nested-prim-sig}}
    \caption{%
      There are two ways we look at the performance of building a term like \mintinline{coq}{projT1 (projT2 ... (projT2 x))} with $n$ \mintinline{coq}{projT2}s:
      we can define a recursive function that computes this term and then use \mintinline{coq}{cbv} to reduce away the recursion and time how long this takes;
      or we can build the term using \LtacTwo\space and then typecheck it.
      These plots display both of these methods and in addition display the time it takes to run \mintinline{coq}{destruct} to break $x$ into its component fields, as a lower bound for how long it takes to prove anything about a nested $\Sigma$ type with $n$ fields.
      The second graph displays the timing with primitive projections turned on.
      Note that the $x$-axis is $10\times$ larger on this plot.%
    }
    \label{fig:timing-make-destruct-nested-sig}
\end{figure}
  ...
  In Coq 8.11, \mintinline{coq}{destruct} is unfortunately still quadratic due to issues with name generation, but the constant factor is much smaller; see \autoref{fig:timing-make-destruct-nested-record} and \coqbug{12271}.
...
Note that the non-monotonic timing is \emph{reproducible} and seems to be due to hitting garbage collection; see \coqissue{12270} for more details.}
...
\end{figure}
  ...
  There's an interesting question, though, of if we can avoid this overhead altogether.

  As seen in \autoref{fig:timing-make-destruct-nested-record}, using records partially avoids the overhead.
  ...
  If your\todoac{not ``your''} proof assistant does not support records out-of-the-box, or you\todoac{not ``you''} want to avoid using them for whatever reason\footnote{Note that the UniMath library~\cite{voevodsky_2015,UniMath,introduction2018Grayson} does this. \minortodo{explain reasoning}}, you\todoac{not ``you''} can instead define intermediate abstraction barriers by hand.
...
\end{minted}
  ...
  Note that this cast node is only present to make explicit the conversion problem that must happen; removing it does not break anything, but then the quadratic cost is hidden in nontrivial substitutions of the \mintinline{coq}{let} binders into the types.
  ...
  \footnote{%
      ...
      One thing that might help would be to have a version of conversion checking that was both memoized and could perform in-place reduction; see \coqissue{12269}.%
  }
...
This is an exception to the rule of opaque abstraction barriers.
...
%    .
    ...
    We have spent some time attempting to divine the appropriate computation rules for pattern-matching constructs, in the hopes of making reasoning with proofs of equality more pleasant.\footnote{See \coqissue{3179} and \coqissue{3119}.}
...
\begin{subappendices}
  \todo{this chapter}
    ...
    This chapter contains a number of miscellaneous fragments of how to design APIs for performance that we could not find a natural place for in any other chapter.
Readers uninterested in the details should feel free to skip over this chapter, as it is somewhat disconnected from the rest of this dissertation.

%\section{Design Decisions in the Category Theory Library}\label{sec:misc:category-theory-design}

All of the design principles in this chapter arose in the course of building the category theory library we implemented in Coq~\cite{category-coq-experience}, introduced in \autoref{sec:category-theory-library}.

\section{Identities vs.\ Equalities; Associators}\label{sec:associators}
...
We begin our discussion in \autoref{sec:categories}\todonz{?} considering a mundane aspect of type definitions that has large consequences for usability and performance.
